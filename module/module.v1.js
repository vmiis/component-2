...
m.set_req=function(){
	...
	m.req={cmd:'query_records',db_pid:_db_pid,sql:sql,sql_n:sql_n,s1:'"'+$('#keyword__ID').val()+'"',I:$('#I__ID').text(),page_size:$('#page_size__ID').val()}
}
//-------------------------------------
...
m.render=function(I){
    ...
    for(var i=0;i<m.records.length;i++){
        ...
        for(var j=0;j<m.field_header.length;j++){
            if(m.field_header[j]!=='_Hidden' && m.field_header[j]!=='_gridHidden'){
                var b=m.field_id[j];
                var value="";
    ...
    $('#grid__ID td').each(function(){
        ...
        if(column_name=='_Form'){
            ...
            $(this).find('u').on('click',function(){
                ...
                if(form_module_name===undefined){
                    ...
                    $vm.load_module_v2(name,$vm.root_layout_content_slot,{record:m.records[I]});
                }
                else{
        ...
        //-------------------------
        if(m.cell_render!==''){ m.cell_render(m.records,row,column_name,$(this),_set_value,'grid'); }
        //-------------------------
    ...
    if($vm.edge==0) $('#grid__ID td').blur(function(){
        ...
        if(m.cell_value_process!=="") value=m.cell_value_process(value,column_name);
...
var _simple_render=function(){
    ...
    //-------------------------
    for(var i=0;i<m.field_header.length;i++){
        if(m.field_header[i]!=='_Hidden' && m.field_header[i]!=='_gridHidden'){
            var print='';
            var header_name=m.field_header[i];
            if(m.field_header[i][0]=='_'){
                print='class=c_print';
            ...
            header_name=header_name.replace(/_/g,' ');
            var header_id=m.field_id[i]; if(m.field_header[i]=='_Form') header_id='_Form';
...
var _row_data=function(I){
    ...
    for(var i=0;i<m.field_id.length;i++){
        var a=m.field_header[i][0];
        var b=m.field_id[i];
		var b2=m.field_id[i].substring(0,3);
        if(m.field_header[i]=='_Hidden' || m.field_header[i]=='_gridHidden' || (b2!='I2_' && a!='_' && b!=="ID" && b!=="DateTime" && b!=="Author") ){
		...
		$vm.load_module_by_name(name,$vm.root_layout_content_slot,
			{
				...
				field_header:_headerFormA,field_id:_headerFormB,cell_render:_cell_render,widthA:_widthA,widthB:_widthB,min_widthA:_min_widthA,min_widthB:_min_widthB,
        ...
        $vm.load_module_by_name(prefix+form_module_name,$vm.root_layout_content_slot,
            {
                ...
                field_header:m.field_header,headerFormB:_headerFormB,
...
$('#D__ID').on('load_quest_form_module',function(event,trigger_parameters){
	...
	if(form_module_name===undefined){
	    ...
	    $vm.load_module_by_name(name,$vm.root_layout_content_slot,
	        {   records:_records,res:_res,I:0,
	            field_header:_headerFormA,field_id:_headerFormB,
	            cell_render:_cell_render,
